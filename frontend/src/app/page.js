...
function BetSuccessModal({ result, match, onClose, theme }) {
  const [confettiPieces] = useState(() =>
    Array.from({ length: 18 }, (_, i) => ({
      ...
      color: i % 3 === 0 ? "#00d4ff" : i % 3 === 1 ? "#7b2ff7" : "#ffd700",
      ...
      <div className="animate-slide-up" style={{
        ...
        <div style={{ padding: 28, textAlign: "center" }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, color: "#00d4ff", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: "#00d4ff", fontFamily: "'Inter', sans-serif" }}>{outcomeName}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: "#f8fafc" }}>
                ${Number(result.amount || 0).toFixed(2)} USDT
              </span>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ fontSize: 16, fontWeight: 800, fontFamily: "'JetBrains Mono', monospace", color: "#4ade80" }}>
                ${Number(result.payout || 0).toFixed(2)} USDT
              </span>
            </div>
          </div>
            ...
            <>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div className="bet-input-wrapper">
                  ...
                  />
                  <span className="bet-input-suffix font-mono">USDT</span>
                </div>
              </div>
...
const LEADERBOARD_FULL = [
  { rank: 1, addr: "0x7F3a...3aB9", winRate: 68, profit: 45230, bets: 84, medal: "🥇", streak: 7 },
  { rank: 2, addr: "0x12Bd...9cF4", winRate: 62, profit: 32105, bets: 71, medal: "🥈", streak: 4 },
  { rank: 3, addr: "0x9A4c...e2D1", winRate: 59, profit: 28450, bets: 63, medal: "🥉", streak: 3 },
  { rank: 4, addr: "0x4B2f...f1A8", winRate: 54, profit: 21000, bets: 58, medal: null, streak: 2 },
  { rank: 5, addr: "0xD31e...7eC2", winRate: 51, profit: 18750, bets: 52, medal: null, streak: 1 },
  { rank: 6, addr: "0xE5a9...b3F7", winRate: 49, profit: 15200, bets: 47, medal: null, streak: 0 },
  { rank: 7, addr: "0xA28d...d1C9", winRate: 47, profit: 11800, bets: 44, medal: null, streak: 0 },
  { rank: 8, addr: "0xC74b...2aE5", winRate: 44, profit: 9600, bets: 39, medal: null, streak: 0 },
  { rank: 9, addr: "0xF1c2...8dB3", winRate: 42, profit: 7400, bets: 35, medal: null, streak: 0 },
  { rank: 10, addr: "0xB39a...5fA1", winRate: 40, profit: 5100, bets: 31, medal: null, streak: 0 },
];
    ...
    <div className="font-sans" style={{ maxWidth: 800, margin: "0 auto" }}>
      <div style={{ marginBottom: 28 }}>
        <h1 className="font-serif" style={{ fontSize: 34, fontWeight: 400, letterSpacing: "-0.01em", marginBottom: 6 }}>
          Global <span className="text-gradient-primary-purple" style={{ fontStyle: "italic" }}>Leaderboard</span>
        </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
        {LEADERBOARD_FULL.slice(0, 3).map((row, idx) => {
          const podiumColors = ["#d4af37", "#C0C0C0", "#CD7F32"];
          const podiumGlows = ["rgba(212,175,55,0.2)", "rgba(192,192,192,0.15)", "rgba(205,127,50,0.15)"];
          return (
            <div key={row.rank} className="card" style={{
              padding: "20px 16px", textAlign: "center",
              borderTop: `2px solid ${podiumColors[idx]}`,
              boxShadow: `0 0 24px ${podiumGlows[idx]}, var(--card-shadow)`,
              order: idx === 0 ? 1 : idx === 1 ? 0 : 2,
            }}>
              <div style={{ fontSize: 28, marginBottom: 8 }}>{row.medal}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700, color: podiumColors[idx], marginBottom: 4 }}>{row.addr}</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "var(--green)", fontFamily: "'JetBrains Mono', monospace", letterSpacing: -0.5 }}>
                +${fmtK(row.profit)}
              </div>
              <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4 }}>{row.winRate}% win · {row.bets} bets</div>
              {row.streak > 0 && (
                <div style={{ display: "inline-flex", alignItems: "center", gap: 4, marginTop: 8, padding: "2px 8px", borderRadius: 99, background: "rgba(255,100,0,0.1)", border: "1px solid rgba(255,100,0,0.25)", fontSize: 10, fontWeight: 700, color: "#ff6400" }}>
                  🔥 {row.streak} streak
                </div>
              )}
            </div>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <div className="font-sans" style={{ padding: "12px 20px", borderBottom: "1px solid var(--border)", display: "grid", gridTemplateColumns: "48px 1fr 80px 80px 100px", gap: 8, fontSize: 10, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.07em" }}>
          <span>Rank</span><span>Wallet</span><span style={{ textAlign: "center" }}>Win %</span><span style={{ textAlign: "center" }}>Bets</span><span style={{ textAlign: "right" }}>Profit</span>
        </div>
        {LEADERBOARD_FULL.map((row, idx) => (
          <div key={row.rank} style={{
            padding: "14px 20px", borderBottom: "1px solid var(--border)",
            display: "grid", gridTemplateColumns: "48px 1fr 80px 80px 100px", gap: 8,
            alignItems: "center", cursor: "pointer", transition: "background 0.15s",
            background: idx < 3 ? "rgba(255,255,255,0.01)" : "transparent"
          }}
            onMouseEnter={e => e.currentTarget.style.background = "var(--card-header-bg)"}
            onMouseLeave={e => e.currentTarget.style.background = idx < 3 ? "rgba(255,255,255,0.01)" : "transparent"}
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: row.medal ? 17 : 13, fontWeight: 700, color: ["#d4af37", "#C0C0C0", "#CD7F32"][row.rank - 1] || "var(--text-muted)" }}>
                {row.medal || row.rank}
              </span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{
                ...
                background: `conic-gradient(from ${parseInt(row.addr.replace(/[^a-fA-F0-9]/g, '').slice(0, 2) || '10', 16) * 1.4}deg, var(--primary), var(--purple), var(--primary))`,
                opacity: 0.8
              }} />
              <div>
                <div className="font-mono" style={{ fontSize: 12, fontWeight: 600, color: "var(--text-primary)" }}>{row.addr}</div>
                <div style={{ fontSize: 10, color: "var(--text-muted)" }}>X Layer · {row.bets} bets{row.streak > 0 ? ` · 🔥 ${row.streak}` : ''}</div>
              </div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div className="font-mono" style={{ fontSize: 12, fontWeight: 700, color: row.winRate >= 60 ? "var(--green)" : row.winRate >= 50 ? "var(--primary)" : "var(--text-secondary)" }}>{row.winRate}%</div>
            ...
            <div className="font-mono" style={{ textAlign: "center", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)" }}>
              {row.bets}
            </div>
            <div className="font-mono" style={{ textAlign: "right", fontSize: 13, fontWeight: 800, color: "var(--green)" }}>
              +${fmtK(row.profit)}
            </div>
          </div>
        ))}
            ...
            {wallet.isConnected ? (
              <button className="btn-ghost" onClick={wallet.disconnect} style={{ gap: 7 }}>
                <div style={{
                  ...
                  background: `conic-gradient(from ${parseInt(wallet.address?.slice(2, 4) || "0", 16) * 1.4}deg, var(--primary), var(--purple), var(--primary))`,
                  flexShrink: 0, boxShadow: "0 0 6px rgba(0,212,255,0.3)"
                }} />
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "36px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 28 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <h1 style={{ margin: 0, lineHeight: 1, letterSpacing: "-0.04em" }}>
              ...
              <span style={{
                ...
                Decentralized FIFA World Cup markets on X Layer.<br />
                Earn USDT, Every bet is an on chain NFT.
              </span>
            </h1>
          </div>
          ...
          <div className="hero-stats-container">
            {[
              ...
              { label: "Blockchain", rawValue: null, display: "X Layer", icon: <Globe size={14} />, color: "var(--primary)" },
